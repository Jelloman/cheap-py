"""Tests for SQLite DAO catalog persistence."""
...
class TestSqliteDao:
    """Test suite for SqliteDao catalog persistence."""

    @pytest.fixture
    async def adapter(self) -> AsyncGenerator[SqliteAdapter, None]:
        """Create an in-memory adapter with schema."""
        adapter = await SqliteAdapter.create(":memory:", init_schema=True)
        yield adapter
        await adapter.close()

    @pytest.fixture
    def dao(self, adapter: SqliteAdapter) -> SqliteDao:
        """Create a DAO instance."""
        return SqliteDao(adapter)

    @pytest.mark.asyncio
    async def test_save_and_load_empty_catalog(
        self, adapter: SqliteAdapter, dao: SqliteDao
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_delete_catalog(self, adapter: SqliteAdapter, dao: SqliteDao) -> None:
    ...
    @pytest.mark.asyncio
    async def test_load_nonexistent_catalog(self, adapter: SqliteAdapter, dao: SqliteDao) -> None:
    ...
    @pytest.mark.asyncio
    async def test_property_type_mapping(self, adapter: SqliteAdapter, dao: SqliteDao) -> None:
        """Test that all property types are correctly mapped."""
        from cheap.db.sqlite.dao import DB_TO_PROPERTY_TYPE, PROPERTY_TYPE_TO_DB
